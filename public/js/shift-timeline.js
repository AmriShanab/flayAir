...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (window.appWorkers) {
        window.appWorkers.forEach(
            (w) => (workerColors[w.id] = generateWorkerColor(w.id))
        );
    }
    ...
    document
        ...
        .addEventListener("click", () => navigateDate(-1));
    document
    ...
    function renderShifts(shifts) {
        ...
        Object.keys(shiftsByWorker).forEach((workerId) => {
            const workerRow = document.querySelector(
                `.worker-row[data-worker-id="${workerId}"]`
            );
            if (!workerRow) return;
    ...
    function createShiftBlock(
        ...
        const borderColor = isBreak
            ? darkenColor("#ff0707ff", 20)
            : darkenColor(workerColor || "#6B7280", 20);
        ...
        <div class="shift-content group">
            <div class="flex justify-between items-start">
                <div class="flex-1 min-w-0">
                    <div class="text-sm font-semibold truncate">${
                        isBreak ? "BREAK" : shift.flight?.flight_number || ""
                    }</div>
                    <div class="text-xs opacity-90">${shift.start_time}-${
            shift.end_time
        }</div>
                </div>
                ${adminControls}
            ...
            ${
                shift.notes
                    ? `<div class="mt-2 text-xs italic line-clamp-2" title="${shift.notes}">💬 ${shift.notes}</div>`
                    : ""
            }
        </div>
    `;
        ...
        if (!isBreak && shift.break_time_start && shift.break_time_end) {
            ...
            breakBlock.style.left =
                (breakStartMin - shiftStartMin) * pxPerMin + "px";
            breakBlock.style.top = "0";
            breakBlock.style.width =
                (breakEndMin - breakStartMin) * pxPerMin + "px";
    ...
    function openEditModal(shift) {
        ...
        if (!modal || !title || !content)
            return console.error("Modal elements missing");
        ...
        const todayFlights = (window.todayFlights || []).filter(
        ...
        content.innerHTML = `
            ...
            <div>
                <label>Notes</label>
                <textarea name="notes" class="w-full p-2 border rounded">${
                    shift.notes || ""
                }</textarea>
            </div>
        ...
        form.addEventListener(
            ...
            async function handler(e) {
                ...
                submitBtn.innerHTML =
                    '<span style="opacity:.9">Updating…</span>';
                ...
                try {
                    ...
                    if (!resp.ok) {
                        ...
                        errorsDiv.textContent =
                            text || `Update failed (status ${resp.status})`;
                        return;
                    }
    ...
    function deleteShift(id) {
        ...
        if (csrfMeta) formData.append("_token", csrfMeta.content);
        fetch(`./admin/shifts/${id}`, {
            method: "POST",
            body: formData,
            headers: { "X-CSRF-TOKEN": csrfMeta ? csrfMeta.content : "" },
        })
            .then(() => loadDataForDate(dateInput.value))
    ...
    function showShiftDetails(shift) {
        ...
        if (!modal || !title || !body || !closeBtn)
            return console.error("Shift details modal elements missing");

        // ✅ Get the flight number safely
        const flightNumber = shift.flight ? shift.flight.flight_number : "N/A";
        ...
        <p><strong>Assigned Flight:</strong> ${flightNumber}</p>
    ...
    function renderFlights(flights) {
        ...
        Object.keys(flightsBySlot).forEach((slot) => {
            flightsBySlot[slot].forEach((f, idx) =>
                createFlightItem(f, parseInt(slot), idx, flightTimeline)
            );
        });
    }
