...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .mark-all-read:hover { background: rgba(26, 86, 219, 0.2); }
        .clear-all { background: transparent; color: #dc3545; border: 1px solid #dc3545; }
        .clear-all:hover { background: #dc3545; color: white; }

        .notification-list { background: white; border-radius: 12px; box-shadow: 0 5px 15px rgba(0,0,0,0.05); overflow: hidden; }
        .notification-item { padding: 20px; border-bottom: 1px solid #f0f4ff; display: flex; align-items: flex-start; transition: background 0.3s; }
        .notification-item:last-child { border-bottom: none; }
        .notification-item:hover { background: #f9fafc; }
        .notification-item.unread { background: #f0f7ff; border-left: 4px solid #1a56db; }

        .notification-icon { width: 50px; height: 50px; border-radius: 50%; background: rgba(26, 86, 219, 0.1); display: flex; justify-content: center; align-items: center; color: #1a56db; font-size: 20px; margin-right: 15px; flex-shrink: 0; }
        ...
        .notification-time { font-size: 14px; color: #888; }
        ...
        /* Responsive */
        @media (max-width: 992px) { .sidebar { width: 80px; } .sidebar-header h2, .menu-item span, .menu-label { display: none; } .menu-item { justify-content: center; padding: 20px; } .menu-item i { margin-right: 0; font-size: 22px; } .main-content { margin-left: 80px; } }
        @media (max-width: 768px) { .main-content { margin-left: 0; padding: 20px; } .sidebar { transform: translateX(-100%); } .sidebar.active { transform: translateX(0); } .header { flex-direction: column; align-items: flex-start; } .user-info { margin-top: 15px; } .menu-toggle { display: block; position: fixed; top: 20px; left: 20px; z-index: 1100; background: #0a2e6f; color: white; width: 45px; height: 45px; border-radius: 50%; display: flex; justify-content: center; align-items: center; box-shadow: 0 3px 10px rgba(0,0,0,0.2); } .page-header { flex-direction: column; align-items: flex-start; gap: 15px; } .notification-actions { width: 100%; justify-content: space-between; } .notification-item { flex-direction: column; } .notification-icon { margin-bottom: 15px; } }
        @media (max-width: 480px) { .notification-actions-item { gap: 8px; } .btn-small { padding: 6px 10px; font-size: 12px; } }
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="/dashboard" class="menu-item"><i class="fas fa-home"></i><span>Dashboard</span></a>
            <a href="/shifts" class="menu-item"><i class="fas fa-calendar-alt"></i><span>My Shifts</span></a>
            <a href="{{ route('worker.notifications') }}" class="menu-item active"><i class="fas fa-bell"></i><span>Notifications</span></a>
            <div class="menu-label">Account</div>
            <a href="{{ route('settings.view') }}" class="menu-item"><i class="fas fa-cog"></i><span>Settings</span></a>
        </div>
    </div>
        ...
        <div class="notifications-container">
            <div class="page-header">
                ...
                <div class="notification-actions">
                    <button class="action-btn mark-all-read"><i class="fas fa-check-double"></i> Mark all as read</button>
                    <button class="action-btn clear-all"><i class="fas fa-trash"></i> Clear all</button>
                </div>
            </div>
                ...
                @forelse($notifications as $notification)
                    <div class="notification-item {{ !$notification->read_at ? 'unread' : '' }}">
                        ...
                        <div class="notification-content">
                            ...
                            <div class="notification-time"><i class="far fa-clock"></i> {{ $notification->created_at->diffForHumans() }}</div>
                            <div class="notification-actions-item">
                               ...
                               @if($notification->is_read == 0 || $notification->is_read == 1)
                                ...
                                @elseif($notification->is_read == 2)
                                    <span class="ack-status"><i class="fas fa-check-circle"></i> Accepted</span>
                                @endif
   ...
   <div id="notificationModal" class="modal">
    <div class="modal-content">
        ...
        <p id="modalMessage"></p>
        <p>
            Start: <span id="modalShiftStart"></span><br>
            End: <span id="modalShiftEnd"></span><br>
        </p>
        <p>Flight No: <span id="modalFlight"></span></p>
    </div>
</div>
    ...
    <style>
        ...
        .modal-content { background: #fff; margin: 10% auto; padding: 20px 25px; border-radius: 10px; max-width: 500px; box-shadow: 0 8px 20px rgba(0,0,0,0.2); animation: slideDown 0.3s ease; }
...
document.querySelectorAll('.btn-dismiss').forEach(btn => {
    ...
    document.querySelector('.mark-all-read')?.addEventListener('click', function() {
        ...
        Object.assign(toast.style, { position: 'fixed', bottom: '20px', right: '20px', backgroundColor: '#10b981', color: 'white', padding: '10px 20px', borderRadius: '6px', boxShadow: '0 4px 10px rgba(0,0,0,0.1)', zIndex: '1000' });
    ...
    // Dismiss
    document.querySelectorAll('.btn-dismiss').forEach(btn => btn.addEventListener('click', function() {
        const item = this.closest('.notification-item');
        item.style.opacity = '0';
        setTimeout(() => item.remove(), 300);
    }));
    ...
    // Accept button
    document.querySelectorAll('.btn-ack').forEach(btn => btn.addEventListener('click', function() {
        const id = this.dataset.id;
        const button = this;
        fetch(`/notification/acknowledge/${id}`, {
            method: 'POST',
            ...
            if(data.success) button.outerHTML = '<span class="ack-status"><i class="fas fa-check-circle"></i> Accepted</span>';
        }).catch(err => console.error(err));
    }));
